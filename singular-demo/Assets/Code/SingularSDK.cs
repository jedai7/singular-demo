...
public class SingularSDK : MonoBehaviour {
    ...
    private const string UNITY_VERSION = "4.3.2";
    ...
    private static SingularConfig BuildSingularConfig() {
        ...
        config.SetValue("shortlinkResolveTimeout", instance.shortlinkResolveTimeout == 0 ?
    ...
    */
    ...
    private void SingularDeviceAttributionCallback(string handlerParamsJson) {
        Debug.Log(string.Format("SingularSDK SingularDeviceAttributionCallback called! message='{0}'", handlerParamsJson));

        if (handlerParamsJson != null)
        {
            Dictionary<string, object> attributes = JsonConvert.DeserializeObject<Dictionary<string, object>>(handlerParamsJson);
